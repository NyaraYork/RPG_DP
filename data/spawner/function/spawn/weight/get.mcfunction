...
# プレイヤーのレベルが指定の範囲内でなければその要素を削除
    execute store result score #MinSpawnableLevel Temp run data get storage spawner: data.SpawnPotentials[-1].Requirements.MinSpawnableLevel
    execute store result score #MaxSpawnableLevel Temp run data get storage spawner: data.SpawnPotentials[-1].Requirements.MaxSpawnableLevel
...
# 時刻が指定の範囲内でなければその要素を削除
    ...
    execute store result score #MinSpawnableTime Temp run data get storage spawner: data.SpawnPotentials[-1].Requirements.MinSpawnableTime
    execute store result score #MaxSpawnableTime Temp run data get storage spawner: data.SpawnPotentials[-1].Requirements.MaxSpawnableTime
...
# 全ての条件を満たしたらWeightの値を合算
    execute store result score #Weight Temp run data get storage spawner: data.SpawnPotentials[-1].Weight
    scoreboard players operation #WeightSum Temp += #Weight Temp
    data modify storage spawner: data.SpawnPotential prepend from storage spawner: data.SpawnPotentials[-1]
