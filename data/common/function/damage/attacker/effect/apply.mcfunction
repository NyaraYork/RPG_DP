#> common:damage/attacker/effect/apply
# 攻撃した時に発動するエフェクトを適用するコマンド

# エフェクトを適用
    execute unless data storage common: EffectsBuf[-1].target.NextID run function common:effect/trigger/attacked
...
# 適用していないエフェクトがなくなるまで再帰
    execute if data storage common: EffectsBuf[] run function common:damage/attacker/effect/apply
